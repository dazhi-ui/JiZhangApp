...
public class  ThePieChare2 extends AppCompatActivity {
    private PieChart pieChart;
    private Context context;
    private Intent intent2;
    private String username;
    private int gongzi=0,jiangjin=0,qita=0;
    private DBOpenMessage dbOpenMessage;
    private TextView gongzitxt,jiangjintxt,qitatxt;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.piechare2);
        pieChart = (PieChart) findViewById(R.id.wenpie2);

        //关于数据库操作
        context=this;
        intent2=getIntent();
        username=intent2.getStringExtra("username");//经检验已经成功
        dbOpenMessage=new DBOpenMessage(ThePieChare2.this,"db_wen2",null,1);
        getMessage1(username);

        gongzitxt=(TextView)findViewById(R.id.wentext21) ;
        jiangjintxt=(TextView)findViewById(R.id.wentext22);
        qitatxt=(TextView)findViewById(R.id.wentext23);

        gongzitxt.setText(Integer.toString(gongzi));
        jiangjintxt.setText(Integer.toString(jiangjin));
        qitatxt.setText(Integer.toString(qita));

        //关于图标的操作
        pieChart.setUsePercentValues(true);//设置value是否用显示百分数,默认为false
        pieChart.setDescription("所有金额收入情况");//设置描述
    ...
    private void getMessage1(String username) {
        ...
        if(cursor!=null){
            while(cursor.moveToNext()){
                ...
                if(message2.userchoice.equals("收入"))
                {
                    if(message2.userkind.equals("工资"))
                    {
                        gongzi+=Integer.parseInt(message2.usermoney);
                    }
                    else if(message2.userkind.equals("奖金"))
                    {
                        jiangjin+=Integer.parseInt(message2.usermoney);
                    }
